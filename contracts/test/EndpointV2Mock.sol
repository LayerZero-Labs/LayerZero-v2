// SPDX-License-Identifier: MIT OR LZBL-1.2
pragma solidity ^0.8.22;

/**
 * @title MockLzEndpoint
 * @dev A very basic mock implementation for testing purposes only
 */
contract MockLzEndpoint {
    // Basic storage variables that will be accessed by the OApp contracts
    address public lzToken;
    address public nativeToken;
    mapping(address => address) public delegates;
    
    constructor() {
        lzToken = address(this);
        nativeToken = address(0);
    }
    
    // Basic function to set delegate
    function setDelegate(address _delegate) external {
        delegates[msg.sender] = _delegate;
    }
    
    // Function to allow sending messages (will be a no-op)
    function send(
        address,  // _receiver
        ...
        // Just return a dummy guid
        return bytes32(uint256(1));
    }
    
    // Add any other functions needed by your specific implementation
    // These are just placeholders to make the contract compile
...
import { MessagingFee, MessagingParams, MessagingReceipt, Origin, ILayerZeroEndpointV2 } from "@layerzerolabs/lz-evm-protocol-v2/contracts/interfaces/ILayerZeroEndpointV2.sol";
