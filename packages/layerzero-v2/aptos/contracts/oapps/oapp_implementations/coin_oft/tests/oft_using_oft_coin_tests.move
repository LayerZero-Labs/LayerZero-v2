...
// valid.
#[test_only]
module oft::oft_using_oft_coin_tests {
    use std::account;
    ...
    use oft::oft_coin::{mint_tokens_for_test, PlaceholderCoin, set_fee_bps, set_rate_limit};
    use oft::oft_store;
    ...
    fun setup(local_eid: u32, remote_eid: u32) {
        ...
        oft::oft_coin::init_module_for_test();

        let remote_oapp = from_address(@2000);
        set_peer(oft_admin, DST_EID, from_bytes32(remote_oapp));
    }

    fun burn_coin_for_test<CoinType>(coin: Coin<CoinType>) {
        if (!account::exists_at(@0xffffff)) {
            create_account_for_test(@0xffffff);
        };
        aptos_account::deposit_coins<CoinType>(@0xffffff, coin);
    }
    ...
    fun test_send_coin() {
        ...
        let coins = mint_tokens_for_test<PlaceholderCoin>(amount);
    ...
    fun test_send_coin_with_fee() {
        ...
        primary_fungible_store::deposit(address_of(alice), fa);
        let bob = from_address(@5678);
        let coins = mint_tokens_for_test(amount);
        ...
        assert!(coin::value(&coins) == 0, 1); // after send balance
        ...
        oft::oft_coin::set_fee_deposit_address(&create_signer_for_test(@oft_admin), @9999);
    ...
    fun test_send_fails_if_insufficient_balance() {
        ...
        primary_fungible_store::deposit(address_of(alice), fa);
        let bob = from_bytes32(from_address(@5678));
        let coins = mint_tokens_for_test(amount_balance);
        aptos_account::deposit_coins<PlaceholderCoin>(address_of(alice), coins);
    ...
    fun test_send_exceed_rate_limit() {
        ...
        let coins = mint_tokens_for_test(amount * 3);
        aptos_account::deposit_coins<PlaceholderCoin>(address_of(alice), coins);
